...
WARNING: Differences are counted as having different characters, i.e. an ambiguity code (e.g. R) is considered different from A, T, C and G (SHOULD UPDATE).

Gap characters ('-') are not considered as differences by default, but this can be changed with the --no_gaps option.
...
def number_of_substitutions(seq1, seq2, no_gaps = False):
	"""
	Count the number of substitutions between two sequences.
	
	Arguments:
		seq1: First sequence (string).
		seq2: Second sequence (string).
		no_gaps: If True, do not count gaps ('-') as differences (default: False).
	
	Returns:
		The number of substitutions as an integer.
	"""
...
def ali_len_msa2pairwise(seq1, seq2, no_gaps = False):
	"""
	Count the length of the alignment between two sequences, ignoring columns where both sequences have gaps.

	Arguments:
		seq1: First sequence (string).
		seq2: Second sequence (string).
		no_gaps: If True, do not count gaps ('-') present in only one of the sequences as differences (default: False).
	Returns:
		The length of the alignment as an integer.
	"""
	assert len(seq1) == len(seq2)
	if no_gaps:
...
def plot_heatmap(dataframe, title, outfile, outformat = "svg", log_scale = False):
	"""
	Plot a heatmap of the given dataframe and save it to a file.
	
	Arguments:
		dataframe: The dataframe to plot (pandas DataFrame).
		title: The title of the plot (string).
		outfile: The output file name (string).
		outformat: The format of the output file (string, default: "svg").
		log_scale: If True, use a logarithmic scale for the color map (default: False).
	
	Returns:
		None, it saves the plot to a file.
	"""
			...
			if seq1_found:
				...
				while j < nb_seq:
					...
					if seq2_found:
						...
						pairwise_subs_num.loc[rec1.id, rec2.id] = subs_num
						pairwise_subs_num.loc[rec2.id, rec1.id] = subs_num
						...
						pairwise_ali_len.loc[rec1.id, rec2.id] = ali_len
						pairwise_ali_len.loc[rec2.id, rec1.id] = ali_len
						# except KeyError:
